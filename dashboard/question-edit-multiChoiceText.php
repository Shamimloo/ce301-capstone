            ...
            <div class="p-3 py-5">
              ...
              if (!isset($_GET["questionID"]) || $_GET["questionID"] == "") {
                ...
                if (isset($_POST["editQuestion"])) {
                  ...
                  if (empty($_POST["editMultiChoiceTextOption"])) {
                    $editMultiChoiceTextOption = null;
                  } else {
                    $editMultiChoiceTextOption = $_POST["editMultiChoiceTextOption"];
                  }

                  //ISSET POST - Multi Choice Text Select Correct Answer
                  if (!isset($_POST["editMultiChoiceTextCorrect"])) {
                    $editMultiChoiceTextCorrect = null;
                  } else {
                    $editMultiChoiceTextCorrect = $_POST["editMultiChoiceTextCorrect"];
                  }

                  // Check if any item in the array is an empty string
                  $hasEmptyString = in_array('', $editMultiChoiceTextOption, true);

                  // Check if any item in the array is empty
                  $hasEmptyValue = false;
                  if (is_array($editMultiChoiceTextOption)) {
                    foreach ($editMultiChoiceTextOption as $item) {
                      if (empty($item)) {
                        $hasEmptyValue = true;
                        break;
                      }
                  ...
                  $isNotLengthFour = (count($editMultiChoiceTextOption) !== 4);

                  if ($editQuestionTitle == "" || $editMultiChoiceTextOption == "" || $editMultiChoiceTextCorrect == "" || $hasEmptyString || $hasEmptyValue || $isNotLengthFour) {
                    ...
                    if (
                      ...
                      for ($i = 0; $i < 4; $i++) { //Insert data for Option
                        if ($editMultiChoiceTextCorrect == $i) {
                          DB::insert('option', [
                            'optionText' => $editMultiChoiceTextOption[$i],
                            'optionCorrect' => 1,
                  ...
                  ?>
                    <div class="row d-flex justify-content-center align-items-center mt-30">
                      ...
                      <div class="col-lg-11 col-md-10 col-sm-10">
                        <input type="text" class="form-control" name="editMultiChoiceTextOption[]" id="multiChoiceTextOption<?php echo $i; ?>" value="<?php echo $queryDBOptionText; ?>" placeholder="Enter Option <?php echo $i + 1; ?>">
                      </div>
                      <div class="col-lg-1 col-md-2 col-sm-2 text-end">
                        <input class="form-check-input" type="radio" name="editMultiChoiceTextCorrect" value="<?php echo $i; ?>" id="multiChoiceTextCorrect<?php echo $i + 1; ?>" <?php
                  ...
                  <div class="d-flex justify-content-between" style="flex-flow: row wrap;">
                    <div class="image-container">
                      ...
                      <?php if ($questionDBqueryCorrectAnswerImage != null) { ?>
                        <div class="col-lg-12 col-md-12 col-sm-12 text-center">
                          <div class="preview-border preview-correct-answer-image d-flex justify-content-center">
                            <?php
                            echo '<div class="my-auto" style="width:90%;"><img src="assets/images/question/' . $questionDBqueryCorrectAnswerImage . '" height="300px" width="300px" /></div>';
                            ?>
                          </div>
                        </div>
                      <?php } ?>
